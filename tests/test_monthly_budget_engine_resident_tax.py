...
def test_default_monthly_resident_tax_is_backward_compatible(fire_and_action):
    ...
    with_default_zero = calculate_monthly_budget(
        fire_result, action_result, monthly_resident_tax=0.0
    )
    ...
    assert "resident_tax_deducted" not in with_default_zero.reasons


def test_monthly_resident_tax_is_subtracted_from_all_three_amounts(fire_and_action):
    fire_result, action_result = fire_and_action

    baseline = calculate_monthly_budget(fire_result, action_result)
    result = calculate_monthly_budget(
        fire_result, action_result, monthly_resident_tax=4.0
    )

    assert result.safe_monthly == round(max(baseline.safe_monthly - 4.0, 0.0), 2)
    assert result.recommended_monthly == round(
        max(baseline.recommended_monthly - 4.0, 0.0), 2
    )
    assert result.max_monthly == round(max(baseline.max_monthly - 4.0, 0.0), 2)
    assert "resident_tax_deducted" in result.reasons


def test_monthly_resident_tax_floors_at_zero(fire_and_action):
    fire_result, action_result = fire_and_action

    result = calculate_monthly_budget(
        fire_result, action_result, monthly_resident_tax=99999.0
    )
...
def test_monthly_resident_tax_combines_with_social_insurance(fire_and_action):
    fire_result, action_result = fire_and_action
    ...
    result = calculate_monthly_budget(
        ...
        monthly_resident_tax=4.0,
    )

    assert result.safe_monthly == round(max(baseline.safe_monthly - 7.0, 0.0), 2)
    assert "social_insurance_deducted" in result.reasons
    assert "resident_tax_deducted" in result.reasons


def test_monthly_resident_tax_does_not_change_guardrail_status(fire_and_action):
    fire_result, action_result = fire_and_action
...
def test_negative_monthly_resident_tax_raises_value_error(fire_and_action):
    fire_result, action_result = fire_and_action

    with pytest.raises(ValueError):
        calculate_monthly_budget(
            fire_result, action_result, monthly_resident_tax=-1.0
        )
